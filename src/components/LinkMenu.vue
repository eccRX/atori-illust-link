...
<template>
  <DropdownMenuRoot>
    <DropdownMenuTrigger
      class="group flex shrink-0 items-center gap-0 rounded-full border border-black/5 bg-white/70 px-2.5 py-2 text-sm font-medium text-gold shadow-sm backdrop-blur transition hover:border-gold/50 hover:bg-white hover:text-gold data-[state=open]:border-gold/50 data-[state=open]:text-gold sm:gap-1.5 sm:px-3 sm:py-1.5"
      aria-label="リンク"
    >
      <svg
        viewBox="0 0 24 24"
    ...
    <DropdownMenuPortal>
      <DropdownMenuContent
        :side-offset="8"
        align="end"
        class="z-50 flex max-h-[75vh] w-72 flex-col overflow-y-auto rounded-2xl border border-black/5 bg-white/95 p-2 shadow-2xl shadow-black/10 ring-1 ring-black/5 backdrop-blur-xl data-[state=open]:animate-fade-in"
      >
        <template v-for="(section, si) in grouped" :key="si">
          ...
          <DropdownMenuLabel
            v-if="section.group"
            class="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-widest text-gold/80"
          >
            {{ section.group }}
          </DropdownMenuLabel>
          ...
          >
            <span
              class="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-black/5 bg-black/[0.03] text-base text-gold transition group-hover/item:scale-110 group-hover/item:text-gold"
            >
